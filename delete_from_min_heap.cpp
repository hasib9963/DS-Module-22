...
void insert_heap(vector<int> &v, int x)
{
    ...
    while (cur_idx != 0)
    {
        ...
        if (v[parent_idx] > v[cur_idx])      // If the parent is greater than the current element
            swap(v[parent_idx], v[cur_idx]); // Swap them
        else
            break;            // If the parent is smaller or equal, stop bubbling up
...
void delete_heap(vector<int> &v)
{
    ...
    while (true)
    {
        ...
        {
            if (v[left_idx] <= v[right_idx] && v[left_idx] < v[cur]) // If the left child is smaller or equal to the right child and smaller than the current element
            {
            ...
            else if (v[right_idx] <= v[left_idx] && v[right_idx] < v[cur]) // If the right child is smaller or equal to the left child and smaller than the current element
            {
            ...
            {
                break; // If both children are larger or equal, stop bubbling down
            }
        }
        else if (left_idx <= last_idx) // If only the left child exists
        {
            if (v[left_idx] < v[cur]) // If the left child is smaller than the current element
            {
                swap(v[left_idx], v[cur]); // Swap the left child with the current element
                cur = left_idx;            // Update the current index to the left child index
            }
            ...
            {
                break; // If the left child is larger or equal, stop bubbling down
            }
        }
        else if (right_idx <= last_idx) // If only the right child exists
        {
            if (v[right_idx] < v[cur]) // If the right child is smaller than the current element
            {
                swap(v[right_idx], v[cur]); // Swap the right child with the current element
                cur = right_idx;            // Update the current index to the right child index
            }
            ...
            {
                break; // If the right child is larger or equal, stop bubbling down
            }
        }
        else
        {
...
int main()
{
    ...
    print_heap(v);  // Print the heap after all insertions
    delete_heap(v); // Delete the root element from the heap
    print_heap(v);  // Print the heap after the deletion
